        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-gray-500 font-medium">
              {profile?.specialty} | {profile?.yearsOfExperience}y Experience
            </p>
          </div>
        </div>
        <Button
      ...
      <section>
        ...
        <div className="grid gap-6">
          ...
          {consultations?.map((consultation: any) => (
            <Card
              key={consultation.id}
              className="overflow-hidden border-gray-100 hover:shadow-lg hover:shadow-gray-200/50 transition-all duration-300"
            >
              <div className="flex flex-col md:flex-row">
                {/* Side Status Bar */}
                <div
                  className={`w-full md:w-2 ${consultation.prescription ? "bg-green-500" : "bg-blue-600"}`}
                />
                ...
                <div className="grow p-6">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center overflow-hidden border">
                        {consultation.patient.profilePic ? (
                          <img
                            src={consultation.patient.profilePic}
                    ...
                    <div
                      className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${consultation.prescription ? "bg-green-50 text-green-600" : "bg-blue-50 text-blue-600"}`}
                    >
                      ...
                      {consultation.patient.historyOfIllness && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {consultation.patient.historyOfIllness
                              .split(",")
                              .map((ill: string, i: number) => (
                                <span
                                  key={i}
                                  className="bg-white px-3 py-1 rounded-lg text-xs font-medium text-gray-600 border shadow-sm"
                                >
                                  {ill.trim()}
                                </span>
                              ))}
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-700 font-medium">
                          {consultation.patient.historyOfSurgery ||
                      ...
                      <div>
                        ...
                        <p
                          className={`text-sm font-medium ${consultation.allergies ? "text-red-500" : "text-gray-700"}`}
                        >
                          {consultation.allergies || "No known allergies"}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    {consultation.prescription?.pdfUrl && (
                      <div className="flex gap-2">
                        <Button
                          size="lg"
                          variant="outline"
                          className="border-gray-200 rounded-xl px-8"
                          asChild
                        >
                          <a
                        ...
                        <Button
                          size="lg"
                          variant="ghost"
                          className="rounded-xl px-4 text-blue-600"
                          onClick={() =>
                            downloadFile(
                              consultation.prescription.pdfUrl,
